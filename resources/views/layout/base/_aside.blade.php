{{-- Aside --}}
<style>
  #kt_aside_menu > .menu-nav > li {
    display: none;
  }
  #kt_aside_menu > .menu-nav > li.show {
...
<div class="aside aside-left {{ Metronic::printClasses('aside', false) }} d-flex flex-column flex-row-auto" id="kt_aside">
  ...
  <div class="brand flex-column-auto {{ Metronic::printClasses('brand', false) }}" id="kt_brand">
    <div class="brand-logo">
      <a href="{{ url('/') }}">
        <h2 class="text-2xl text-white text-center font-black">JFM</h2>
...
<script>
  $(function() {
    var menuSide = $('#kt_aside_menu');
    menuSide.find('.menu-nav').children('li').each((el, i) => {
      if($(i).hasClass('show')) {
        // $(i).show();
      }
    });
